...
### Changed
- Enhanced `/version-update` copyright year update logic with explicit file locations (LICENSE, README, index.html header/footer, server templates) and year range format guidance.
- Rotated all workflow filenames to v1.18.0.

## [1.17.1] - 2026-01-06

### Changed
- Enhanced `README.md` with detailed documentation for the `/summarize-progress` and `/catch-up` workflows.
...
### Added
- Created `/summarize-progress-v1.14.0` workflow for generating comprehensive status reports for cross-platform AI context sharing.
- Created `/catch-up-v1.14.0` workflow for quickly syncing agent state by reading latest brain artifacts.
- Both new workflows follow the established versioned filename convention.
